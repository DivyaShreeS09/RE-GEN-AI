...
def analyze_impact(water_saved_liters: float, energy_saved_kwh: float, waste_value_inr: float,
                   fuel_co2_kg: float = 0.0) -> dict:
    ...
    total_co2_saved_kg    = round(water_co2_saved + energy_co2_saved + fuel_co2_kg, 2)
    ...
    annual_projections = {
        ...
        "note": "Estimated annual projection — assumes same weekly loss rate sustained for 52 weeks with no intervention.",
    }
    ...
    sdg_alignment = [
        {
            "goal": "6", "title": "Clean Water & Sanitation", "relevance": "High",
            "contribution": (
                f"Detecting and fixing {water_saved_liters} L/week of night-flow leakage reduces freshwater "
                f"stress. Equivalent to conserving {round(water_saved_liters * WEEKS_PER_YEAR)} L/year — a direct "
                "contribution to safe water management targets."
            ),
        },
        {
            "goal": "7", "title": "Affordable & Clean Energy", "relevance": "High",
            "contribution": (
                f"Eliminating {energy_saved_kwh} kWh of after-hours energy waste/week reduces the electricity "
        ...
        {
            "goal": "13", "title": "Climate Action", "relevance": "Medium",
            "contribution": (
                ...
                "electricity use. Directly addresses the organisation's carbon footprint."
            ),
        },
    ]
    ...
    reasoning_trace = [
        ...
        f"Step 2b — Fuel CO2: {round(fuel_co2_kg, 2)} kg CO2 (from fuel emission factors)." if fuel_co2_kg else "Step 2b — Fuel CO2: 0 kg (no fuel data provided).",
        ...
        f"Step 8  — Water scarcity multiplier: x{WATER_SCARCITY_INDEX} applied for regional scarcity index.",
    ...
    return {
        ...
        "fuel_co2_kg":                round(fuel_co2_kg, 2),
